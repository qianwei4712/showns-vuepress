...
<li><a href="#t2">OpenFegin 使用</a></li>
<li><a href="#t3">自定义 FeignClient</a></li>
<li><a href="#t4">OkHttpClient</a></li>
<li><a href="#te">参考文章</a></li>
</ul>
</div>
<h2 id="前言" tabindex="-1"><a class="header-anchor" href="#前言" aria-hidden="true">#</a> <span id="t1">前言</span></h2>
<p><em>2021.10.25，spring cloud 以前只零碎用过几个组件，这次系统学一遍</em></p>
...
<hr>
<p><strong>代码仓库 openFegin-demo 分支</strong> ：<a href="https://gitee.com/learning-use-cases/spring-cloud-demo/tree/openFegin-demo/" target="_blank" rel="noopener noreferrer">Learning Use Cases/Spring Cloud Demo - Gitee.com<ExternalLinkIcon/></a></p>
<p>官方文档：<a href="https://spring.io/projects/spring-cloud-openfeign#overview" target="_blank" rel="noopener noreferrer">Spring Cloud OpenFeign<ExternalLinkIcon/></a></p>
<p>本文介绍 spring cloud 远程服务调用组件 OpenFegin。</p>
<blockquote>
<p>Fegin 是一个声明式 Web 服务客户端，只需要创建一个接口并加上注解，即可通过注册中心被调用。</p>
</blockquote>
<p>看几个层次关系。</p>
<ol>
<li>Fegin 内置了 Ribbon ，实现了客户端负载均衡，调用注册中心获取服务列表。</li>
<li>Fegin 自己实现了声明式注解定义接口，通过这个接口就可以调用注册中心服务。</li>
<li>OpenFegin 在 Fegin 的基础上，实现了 Spring MVC 的支持，通过动态代理产生实现类。</li>
</ol>
<br/>
<h2 id="openfegin-使用" tabindex="-1"><a class="header-anchor" href="#openfegin-使用" aria-hidden="true">#</a> <span id="t2">OpenFegin 使用</span></h2>
<p>添加依赖：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Application 添加注解：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>对应服务控制器，客户端编写 Fegin 接口：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@FeignClient</span><span class="token punctuation">(</span>value <span class="token operator">=</span> <span class="token string">"PAYMENT-SERVICE"</span><span class="token punctuation">)</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>对应远程服务端控制器：</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>调用结果：</p>
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/images/image-20211023215411654.png" alt="image-20211023215411654"></p>
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/images/image-20211023215439456.png" alt="image-20211023215439456"></p>
<p>可见，默认依然是轮询策略。</p>
<br/>
<h2 id="自定义-feignclient" tabindex="-1"><a class="header-anchor" href="#自定义-feignclient" aria-hidden="true">#</a> <span id="t3">自定义 FeignClient</span></h2>
<p><code v-pre>@FeignClient</code> 注解的 configuration 参数，默认是通过 <code v-pre>FeignClientsConfiguration</code> 类定义的，可以配置 Client，Contract，Encoder/Decoder 等。</p>
<p><code v-pre>FeignClientsConfiguration</code> 类中的配置方法及默认值如下：</p>
<ul>
<li><strong>feignContract</strong>: SpringMvcContract</li>
<li><strong>feignDecoder</strong>: ResponseEntityDecoder</li>
<li><strong>feignEncoder</strong>: SpringEncoder</li>
<li><strong>feignLogger</strong>: Slf4jLogger</li>
<li><strong>feignBuilder</strong>: Feign.Builder</li>
<li><strong>feignClient</strong>: LoadBalancerFeignClient（开启Ribbon时）或默认的HttpURLConnection</li>
</ul>
<p>在 <code v-pre>ComponentScan</code> 外进行配置：</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>然后在<code v-pre>@FeignClient</code> 注解时使用：</p>
...
<span class="token keyword">public</span> <span class="token keyword">interface</span> <span class="token class-name">PaymentServiceFegin</span> <span class="token punctuation">{</span>
    ...
    <span class="token class-name">Result</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">PaymentDTO</span><span class="token punctuation">></span></span> <span class="token function">getById</span><span class="token punctuation">(</span><span class="token annotation punctuation">@RequestParam</span><span class="token punctuation">(</span><span class="token string">"id"</span><span class="token punctuation">)</span> <span class="token class-name">Long</span> id<span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>也可以使用 yml 进行配置：</p>
...
<h2 id="okhttpclient" tabindex="-1"><a class="header-anchor" href="#okhttpclient" aria-hidden="true">#</a> <span id="t4">OkHttpClient</span></h2>
<p>Feign 最终发送 Request 请求以及接收 Response 响应都是由 Client 组件来完成的。</p>
<p>首先说明一点，默认的 HttpURLConnection 存在一些问题，一般推荐使用 <code v-pre>OkHttpClient</code> 或者自己封装。</p>
<p>例如，默认的 Client 如下：</p>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependency</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>添加配置：</p>
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">feign</span><span class="token punctuation">:</span>
  <span class="token key atrule">okhttp</span><span class="token punctuation">:</span>
    <span class="token key atrule">enabled</span><span class="token punctuation">:</span> <span class="token boolean important">true</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>超时配置：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@Configuration</span>
...
<p><strong>可见 OkHttp 配置是全局生效的，这里需要注意下。</strong></p>
</blockquote>
<br/>
<h2 id="参考文章" tabindex="-1"><a class="header-anchor" href="#参考文章" aria-hidden="true">#</a> <span id="te">参考文章</span></h2>
<p><a href="https://www.bilibili.com/video/BV18E411x7eT?p=36" target="_blank" rel="noopener noreferrer">尚硅谷SpringCloud框架开发教程(SpringCloudAlibaba微服务分布式架构丨Spring Cloud)_哔哩哔哩_bilibili<ExternalLinkIcon/></a></p>
<p><a href="https://blog.csdn.net/taiyangdao/article/details/81359394" target="_blank" rel="noopener noreferrer">Spring Cloud OpenFeign详解_成长的足迹-CSDN博客_openfeign<ExternalLinkIcon/></a></p>
<p><a href="https://blog.csdn.net/u010277958/article/details/88730889" target="_blank" rel="noopener noreferrer">Feign-使用HttpClient和OkHttp_u010277958的博客-CSDN博客_feign okhttp<ExternalLinkIcon/></a></p>
<p><a href="https://www.cnblogs.com/zk-blog/p/12465951.html" target="_blank" rel="noopener noreferrer">使用HttpClient和OkHttp调用服务的区别（附示例代码） - 骑着蜗牛看海呀 - 博客园 (cnblogs.com)<ExternalLinkIcon/></a></p>
<p><a href="https://segmentfault.com/a/1190000039889836?utm_source=sf-hot-article" target="_blank" rel="noopener noreferrer">Spring Cloud OpenFeign入门和实战 - SegmentFault 思否<ExternalLinkIcon/></a></p>
</div></template>
