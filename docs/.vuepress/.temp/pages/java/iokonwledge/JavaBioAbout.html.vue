...
<li><a href="#%E4%BC%A0%E7%BB%9F%E7%9A%84BIO%E9%80%9A%E4%BF%A1%E6%96%B9%E5%BC%8F">传统的BIO通信方式</a>
<ul>
<li><a href="#BIO%E7%9A%84%E9%97%AE%E9%A2%98">BIO的问题</a></li>
<li><a href="#%E5%A4%9A%E7%BA%BF%E7%A8%8B%E4%BC%98%E5%8C%96">多线程优化-伪异步</a></li>
</ul>
</li>
<li><a href="#BIO%E4%BB%A3%E7%A0%81%E6%A8%A1%E6%8B%9F">BIO 代码模拟</a>
<ul>
<li><a href="#%E4%B8%80Client%E3%80%81%E4%B8%80Server">一Client、一Server</a></li>
<li><a href="#%E5%A4%9AClient%E3%80%81%E4%B8%80Server">多Client、一Server</a></li>
<li><a href="#%E4%BC%AA%E5%BC%82%E6%AD%A5%E5%AE%9E%E7%8E%B0">伪异步实现</a></li>
</ul>
</li>
<li><a href="#BIO%E9%97%AE%E9%A2%98%E6%A0%B9%E6%BA%90">BIO 问题根源</a></li>
<li><a href="#%E5%8F%82%E8%80%83%E6%96%87%E7%AB%A0">参考文章</a></li>
</ul>
</div>
<h2 id="开场废话" tabindex="-1"><a class="header-anchor" href="#开场废话" aria-hidden="true">#</a> <span id="开场废话">开场废话</span></h2>
<p><code v-pre>2022.04.03</code></p>
<p>BIO 就是 blocking IO，同步并阻塞（传统阻塞型），它是最简单的IO工作方式，服务器实现模式为<strong>一个连接一个线程</strong>。</p>
<blockquote>
<p>应用程序向操作系统请求网络IO操作，这时应用程序会一直等待；另一方面，操作系统收到请求后，也会等待，直到网络上有数据传到监听端口；</p>
<p>操作系统在收集数据后，会把数据发送给应用程序；最后应用程序受到数据，并解除等待状态。</p>
</blockquote>
<p>类似就是这样：</p>
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/202204/image-20220403094819434.png" alt=""></p>
<br>
<h2 id="传统的bio通信方式" tabindex="-1"><a class="header-anchor" href="#传统的bio通信方式" aria-hidden="true">#</a> <span id="传统的BIO通信方式">传统的BIO通信方式</span></h2>
...
<br>
<h3 id="bio-的问题" tabindex="-1"><a class="header-anchor" href="#bio-的问题" aria-hidden="true">#</a> <span id="BIO的问题">BIO 的问题</span></h3>
<p>这里可能需要着重解释。</p>
<p>对我们这些习惯使用 Spring 全家桶的初级玩家，虽然能看懂这在说什么，但是却无法理解到位。因为我们习惯使用的都是 JDK 8。</p>
<blockquote>
<p><strong>正在的传统 BIO 模式下，应该说阻塞的其实是主线程</strong></p>
</blockquote>
<p>举个例子：</p>
<ul>
<li><strong>我们写了一个服务端，监听 8090 端口</strong></li>
<li><strong>那么在BIO的限制下，这个 8090 端口一次只能连接一个请求</strong></li>
<li><strong>这时候如果有多个客户端请求，同时只能处理一个请求</strong></li>
</ul>
<p>这显然是有问题的。</p>
<br>
<h3 id="多线程优化-伪异步" tabindex="-1"><a class="header-anchor" href="#多线程优化-伪异步" aria-hidden="true">#</a> <span id="多线程优化">多线程优化-伪异步</span></h3>
<p>这么严重的问题，大佬们肯定是会想办法解决的，所以就有了多线程处理，伪异步方式。</p>
<p>这个方式可以在 BIO 下实现多客户端连接，实际上是</p>
<ul>
<li><strong>主线程 8090 不负责处理实际业务，只负责接收连接请求</strong></li>
<li><strong>主线程接收到连接请求后，再创建一个新线程，新线程负责和请求建立长连接</strong></li>
<li><strong>这样就达到了异步的实现</strong></li>
</ul>
<img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/202204/java-io-bio-2.png" style="zoom:80%;" />
<p>当然，应用层面的解决方案都不能实际解决问题，只能延迟问题暴露的时间。</p>
<p>这种方式的问题在于：</p>
<ol>
<li><strong>服务器虽然是多线程处理业务，但是操作系统通知accept()的方式还是单个的</strong> ， 所以操作系统接收数据依然是一个一个来的。</li>
<li>长链接会占用线程资源，操作系统的线程是有限的，但是 <strong>线程开的越多，CPU切换线程花费的时间就越长</strong> 。</li>
<li>JVM 创建线程是需要分配空间的，也是很费性能的</li>
</ol>
<blockquote>
<p><strong>所以，BIO真正的问题其实不是阻塞等待，而是操作系统层面，对 accept()、read() 的操作点都是被阻塞</strong></p>
</blockquote>
<br>
<h2 id="bio-代码模拟" tabindex="-1"><a class="header-anchor" href="#bio-代码模拟" aria-hidden="true">#</a> <span id="BIO代码模拟">BIO 代码模拟</span></h2>
<h3 id="一client、一server" tabindex="-1"><a class="header-anchor" href="#一client、一server" aria-hidden="true">#</a> <span id="一Client、一Server">一Client、一Server</span></h3>
<p>传统基础一对一 socket：</p>
...
<h3 id="多client、一server" tabindex="-1"><a class="header-anchor" href="#多client、一server" aria-hidden="true">#</a> <span id="多Client、一Server">多Client、一Server</span></h3>
<p>两个线程来发起请求，但是服务端只有一个接受线程：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">IOException</span> <span class="token punctuation">{</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">IOException</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>所以只有第一个线程结束后，才能接收第二个请求的连接：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>===服务端启动===
...
<h3 id="伪异步实现" tabindex="-1"><a class="header-anchor" href="#伪异步实现" aria-hidden="true">#</a> <span id="伪异步实现">伪异步实现</span></h3>
<p>上面已经说过了，弄个线程池来接收请求链接。</p>
<p>客户端代码不变，服务端修改为线程池进行业务处理：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code> <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">IOException</span> <span class="token punctuation">{</span>
     <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">"===服务端启动==="</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
     <span class="token comment">//1.定义一个ServerSocket对象进行服务端的端口注册</span>
     <span class="token class-name">ServerSocket</span> ss <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">ServerSocket</span><span class="token punctuation">(</span><span class="token number">9999</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<h2 id="bio-问题根源" tabindex="-1"><a class="header-anchor" href="#bio-问题根源" aria-hidden="true">#</a> <span id="BIO问题根源">BIO 问题根源</span></h2>
<p>可以看到，打印出来的输出数据，端口已经是随机的了。</p>
<p>到这里也可以看到：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code> <span class="token comment">//2. 监听客户端的Socket连接请求</span>
 <span class="token class-name">Socket</span> socket <span class="token operator">=</span> ss<span class="token punctuation">.</span><span class="token function">accept</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>这一步是在主线程内的，不同线程间的切换要向CPU抢资源</p>
<p><strong>所以还没有建立连接的新请求，都必须排队进行 accept</strong></p>
<hr>
<p>下面开始复制大佬的文章：</p>
...
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/202204/java-io-bio-4.png" alt=""></p>
<ul>
<li>注意，是询问操作系统。也就是说socket套接字的IO模式支持是基于操作系统的，那么自然同步IO/异步IO的支持就是需要操作系统级别的了。如下图:</li>
</ul>
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/202204/java-io-bio-5.png" alt=""></p>
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/202204/java-io-bio-6.png" alt=""></p>
<p>如果操作系统没有发现有套接字从指定的端口X来，那么操作系统就会等待。</p>
<p>这样serverSocket.accept()方法就会一直等待。</p>
<p>这就是为什么accept()方法为什么会阻塞: 它内部的实现是使用的操作系统级别的同步IO</p>
<br/>
<h2 id="参考文章" tabindex="-1"><a class="header-anchor" href="#参考文章" aria-hidden="true">#</a> <span id="参考文章">参考文章</span></h2>
<ul>
<li><a href="https://www.pdai.tech/md/java/io/java-io-bio.html" target="_blank" rel="noopener noreferrer">Java IO - BIO 详解 | Java 全栈知识体系 (pdai.tech)<ExternalLinkIcon/></a></li>
