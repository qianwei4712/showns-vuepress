...
<li><a href="#NIO%E4%BB%8B%E7%BB%8D">NIO 简单介绍</a>
<ul>
<li><a href="#NIO%E4%BB%A3%E7%A0%81%E7%A4%BA%E4%BE%8B">NIO 代码示例</a></li>
<li><a href="#%E6%89%A7%E8%A1%8C%E6%B5%81%E7%A8%8B%E7%AE%80%E4%BB%8B">执行流程简介</a></li>
</ul>
</li>
<li><a href="#Buffer">Buffer 缓冲区详解</a></li>
<li><a href="#Channel">Channel 通道详解</a></li>
<li><a href="#Selector">Selector 选择器详解</a></li>
<li><a href="#%E5%8F%82%E8%80%83%E6%96%87%E7%AB%A0">参考文章</a></li>
</ul>
</div>
<h2 id="开场废话" tabindex="-1"><a class="header-anchor" href="#开场废话" aria-hidden="true">#</a> <span id="开场废话">开场废话</span></h2>
<p><code v-pre>2022.04.05</code></p>
<p>Java 为了解决 BIO 中，<code v-pre>serverSocket.accept()</code> 阻塞的问题，在 JDK1.4 推出了 NIO 弥补不足。</p>
<p>Java NlO (New lO）也有人称之为 java non-blocking IO。<strong>NIO 可以完美得替代原本的 IO API，但是实现方式完全不一样。</strong></p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>NIO 可以理解为非阻塞IO，传统IO 的read和write只能阻塞执行，线程在读写期间不能干其他事倩，比如调用socket. read(）时，如果服务器一直没有数据传输过来，线程就一直阻塞;
...
<h2 id="nio-简单介绍" tabindex="-1"><a class="header-anchor" href="#nio-简单介绍" aria-hidden="true">#</a> <span id="NIO介绍">NIO 简单介绍</span></h2>
<p>标准IO 是对字节流的读写，在进行IO之前，首先创建一个流对象，流对象进行读写操作都是按字节 ，一个字节一个字节的来读或写。</p>
<p><strong>NIO 把IO抽象成块</strong> ，类似磁盘的读写，每次IO操作的单位都是一个块，块被读入内存之后就是一个byte[]，NIO一次可以读或写多个字节，效率也高很多。</p>
<p>NIO有三大核心部分：<code v-pre>Channel（通道）</code>,<code v-pre>Buffer(缓冲区）</code>,<code v-pre>Selector（选择器）</code></p>
<blockquote>
<p><strong>数据总是从 Channel(通道）读取到 Buffer(缓冲区）中，或者从缓冲区写入到通道中。</strong></p>
<p><strong>Selector(选择器）用于监听多个通道的事件（比如：连接请 求，数据到达等），因此使用单个线程就可以监听多个客户端通道</strong></p>
</blockquote>
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/202204/image-20220407234615866.png" alt=""></p>
<br/>
<h4 id="buffer-缓冲区" tabindex="-1"><a class="header-anchor" href="#buffer-缓冲区" aria-hidden="true">#</a> Buffer 缓冲区</h4>
<p>发送给一个通道的所有数据都必须首先放到缓冲区中，同样地，从通道中读取的任何数据都要先读到缓冲区中。</p>
<p>也就是说，不会直接对通道进行读写数据，而是要先经过缓冲区。</p>
<p>缓冲区实质上是一个数组，但它不仅仅是一个数组。缓冲区提供了对数据的结构化访问，而且还可以跟踪系统的读/写进程。</p>
<br/>
<h4 id="channel-通道" tabindex="-1"><a class="header-anchor" href="#channel-通道" aria-hidden="true">#</a> Channel 通道</h4>
<p>通道 Channel 是对原 I/O 包中的流的模拟，可以通过它读取和写入数据。</p>
<p>通道与流的不同之处在于，流只能在一个方向上移动(一个流必须是 InputStream 或者 OutputStream 的子类)，<strong>而通道是双向的，可以用于读、写或者同时用于读写。</strong></p>
<p>通道包括以下类型:</p>
<ul>
...
<br/>
<h4 id="selector-选择器" tabindex="-1"><a class="header-anchor" href="#selector-选择器" aria-hidden="true">#</a> Selector 选择器</h4>
<p>Selector 是一个 java NIO组件，可以能够检查一个或多个NIO通道，并确定哪些通道已经准备好进行读取或写入。</p>
<p>这样，一个单独的线程可以管理多个channel，从而管理多个网络连接，提高效率</p>
<br/>
<h3 id="nio-代码示例" tabindex="-1"><a class="header-anchor" href="#nio-代码示例" aria-hidden="true">#</a> <span id="NIO代码示例">NIO 代码示例</span></h3>
<p>服务端代码如下，主线程，启动监听：</p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//处理数据</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//写数据</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//发送数据</span>
...
<h3 id="执行流程简介" tabindex="-1"><a class="header-anchor" href="#执行流程简介" aria-hidden="true">#</a> <span id="执行流程简介">执行流程简介</span></h3>
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/202204/IO 分类及区别.png" alt=""></p>
<br/>
<h2 id="buffer-缓冲区详解" tabindex="-1"><a class="header-anchor" href="#buffer-缓冲区详解" aria-hidden="true">#</a> <span id="Buffer">Buffer 缓冲区详解</span></h2>
<p>抽象类 <code v-pre>java.nio.Buffer</code> 是所有 NIO 缓冲区实现类的父类。</p>
<p>缓冲区包括以下类型： <code v-pre>ByteBuffer</code>、<code v-pre>CharBuffer</code>、<code v-pre>ShortBuffer</code>、<code v-pre>IntBuffer</code>、<code v-pre>LongBuffer</code>、<code v-pre>FloatBuffer</code>、<code v-pre>DoubleBuffer</code></p>
<p>下面以 <strong>ByteBuffer</strong> 为例，缓冲区的创建方法用的是：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 创建一个容量为capacity的 ByteBuffer 对象</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>最后还是回到  <strong>ByteBuffer</strong> 的构造方法：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 使用给定的标记、位置、限制、容量、后备数组和数组偏移量创建一个新缓冲区</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Buffer中的重要概念和字段：</p>
<ul>
<li>
<p><strong>容量（capacity)</strong>：作为一个内存块，Buffer具有一定的固定大小，也称为”容量”，缓冲区容量不能为负，并且创建后不能更改（<strong>因为它是数组</strong>）。</p>
</li>
<li>
<p><strong>限制（limit)</strong>：表示缓冲区中可以操作数据的大小（limit后数据不能进行读写）。缓冲区的限制不能为负，并且不能大于其容量。</p>
<ul>
<li><strong>写入模式，限制等于buffer 的容量。读取模式下，limit 等于写入的数据量。</strong></li>
</ul>
</li>
<li>
<p><strong>位置(position)</strong>：下一个要读取或写入的数据的索引。缓冲区的位置不能为负，并且不能大于其限制</p>
</li>
<li>
<p><strong>标记（mark）与重置（reset)</strong>：标记是一个索弓l，通过Buffer中的mark(）方法指定Buffer中一个特定的 position，之后可以通过调用reset(）方法恢复到这个position。</p>
<ul>
<li><strong>标记、位置、限制、容量遵守以 T 不变式：<code v-pre>0&lt;=mark&lt;=position&lt;=limit&lt;=capacity</code></strong></li>
</ul>
</li>
</ul>
<h3 id="缓冲区流程图解" tabindex="-1"><a class="header-anchor" href="#缓冲区流程图解" aria-hidden="true">#</a> 缓冲区流程图解</h3>
<ul>
<li>capacity: 最大容量；</li>
...
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/202204/1bea398f-17a7-4f67-a90b-9e2d243eaa9a.png" alt=""></p>
<p>② 从输入通道中读取 5 个字节数据写入缓冲区中，此时 position 移动设置为 5，limit 保持不变。</p>
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/202204/80804f52-8815-4096-b506-48eef3eed5c6.png" alt=""></p>
<p>③ 在将缓冲区的数据写到输出通道之前，需要先调用 flip() 方法，这个方法将 limit 设置为当前 position，并将 position 设置为 0。</p>
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/202204/952e06bd-5a65-4cab-82e4-dd1536462f38.png" alt=""></p>
<p>④ 从缓冲区中取 4 个字节到输出缓冲中，此时 position 设为 4。</p>
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/202204/b5bdcbe2-b958-4aef-9151-6ad963cb28b4.png" alt=""></p>
<p>⑤ 最后需要调用 clear() 方法来清空缓冲区，此时 position 和 limit 都被设置为最初位置。</p>
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/202204/67bf5487-c45d-49b6-b9c0-a058d8c68902.png" alt=""></p>
<h3 id="buffer-常见方法" tabindex="-1"><a class="header-anchor" href="#buffer-常见方法" aria-hidden="true">#</a> buffer 常见方法</h3>
<table>
<thead>
<tr>
<th>方法</th>
...
<tr>
<td>Buffer flip(）</td>
<td>为将缓冲区的界限设置为当前位置，并将当前位置重置为0</td>
</tr>
<tr>
<td>int capacity(）</td>
...
<h3 id="数据操作方法" tabindex="-1"><a class="header-anchor" href="#数据操作方法" aria-hidden="true">#</a> 数据操作方法</h3>
<p><strong>取获取Buffer中的数据</strong></p>
<ul>
<li>get()：读取单个字节</li>
<li>get (byte［〕dst)：批量读取多个字节到dst中</li>
<li>get(int index)：读取指定索引位置的字节（不会移动position)</li>
</ul>
<p><strong>放入数据到Buffer中</strong></p>
<ul>
<li>put(byte b):将给定单个字节写入缓冲区的当前位置</li>
<li>put (byte[] src):将src中的字节写入缓存区的当前位置</li>
<li>put(int index,byte b)L将指定字节写入缓存区的索引位置（不会移动position）</li>
</ul>
<br/>
<h2 id="channel-通道详解" tabindex="-1"><a class="header-anchor" href="#channel-通道详解" aria-hidden="true">#</a> <span id="Channel">Channel 通道详解</span></h2>
<p>通道（Channel)：由 java.nio.channels 包定义的。Channel表示IO源与目标打开的连接。</p>
<p><strong>Channel 类似于传统的“流”。只不过Channel本身不能直接访问数据，Channel只能与Buffer进行交互。</strong></p>
<blockquote>
<p>BlO 中的 stream 是单向的，只能用来读或者写，而 NIO 的 Channel 是双向的。</p>
</blockquote>
<p><strong>常用的 Channel 实现类</strong></p>
<ul>
<li>FileChannel：用于读取、写入、映射和操作文件的通道</li>
<li>DatagramChannel：通过UDP读写网络中的数据通道</li>
<li>SocketChannel：通过TCP读写网络中额数据</li>
<li>ServerSocketChannel：可以监听新进来的TCP连接，对每一个新进来的连接都会创建一个 SocketChannel。
<ul>
<li>【ServerSocketChannel类似ServerSocket，SocketChannel类似Socket】</li>
</ul>
</li>
</ul>
<br/>
<h3 id="通道使用" tabindex="-1"><a class="header-anchor" href="#通道使用" aria-hidden="true">#</a> 通道使用</h3>
<p>获取通道的一种方式是对支持通道的对象调用 <code v-pre>getChannel()</code> 方法，支持这个方法的类如下：</p>
<ul>
<li>
<p>本地io：<code v-pre>FileInputStreanm/FileOutputStream</code>、<code v-pre>RandomAccessFile</code></p>
</li>
<li>
<p>网络io：<code v-pre>Socket</code>、<code v-pre>ServerSocket</code>、<code v-pre>DatagramSocket</code></p>
</li>
</ul>
<h4 id="文件读写" tabindex="-1"><a class="header-anchor" href="#文件读写" aria-hidden="true">#</a> 文件读写</h4>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="通道复制" tabindex="-1"><a class="header-anchor" href="#通道复制" aria-hidden="true">#</a> 通道复制</h4>
<p><code v-pre>transferForm</code> 、<code v-pre>transferTo</code> 两个方法，看代码就行</p>
...
<h2 id="selector-选择器详解" tabindex="-1"><a class="header-anchor" href="#selector-选择器详解" aria-hidden="true">#</a> <span id="Selector">Selector 选择器详解</span></h2>
<p>选择器（Selector）是SeIectabIe ChanneI对象的多路复用器，Selector可以同时监控多个 SelectableChannel 的IO状况，</p>
<p>也就是说，<strong>利用 Selector 可使一个单独的线程管理多个 Channel。</strong></p>
<blockquote>
<p>Selector 能够检测多个注册的通道上篡若有事件发生（注意：多个Channel以事件的方式可以注册到同一个 Selector)；</p>
<p>如果有事件发生，便获取事件然后针对每个事件进行相应的处理。这样就可以只用一个单线程去管理多个通道，也就是管理多个连接和请求。</p>
</blockquote>
<blockquote>
<p>只有在连接／通道真正有读写事件发生时，才会进行读写，就大大地减少了系统开销，并且不必为每个连接都创建一个线程，不用去维护多个线程 避免了多线程之间的上下文切换导致的开销</p>
</blockquote>
<br/>
<h3 id="selector-使用" tabindex="-1"><a class="header-anchor" href="#selector-使用" aria-hidden="true">#</a> Selector 使用</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>向选择器注册通道：<code v-pre>SelectableChannel.register(Selector sel,int ops);</code></p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>通道必须配置为非阻塞模式，否则使用选择器就没有任何意义了</strong>，</p>
<p>因为如果通道在某个事件上被阻塞，那么服务器就不能响应其它事件，必须等待这个事件处理完毕才能去处理其它事件，显然这和选择器的作用背道而驰。</p>
<p>ops指定，可以监听的事件类型（用可使用Selection Key的四个常量表示）:</p>
<ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="事件监听" tabindex="-1"><a class="header-anchor" href="#事件监听" aria-hidden="true">#</a> 事件监听</h3>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">int</span> num <span class="token operator">=</span> selector<span class="token punctuation">.</span><span class="token function">select</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>使用 select() 来监听到达的事件，它会一直阻塞直到有至少一个事件到达。</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>代码就不写了，上门的示例包含了全流程
...
<ul>
<li><a href="https://www.bilibili.com/video/BV1gz4y1C7RK?p=2&amp;spm_id_from=pageDriver" target="_blank" rel="noopener noreferrer">黑马Java-IO模式精讲(AIO&amp;BIO&amp;NIO)，Java-IO基础+高级+实战全套教程，黑马程序员行业大牛深度精讲_哔哩哔哩_bilibili<ExternalLinkIcon/></a></li>
<li><a href="https://www.pdai.tech/md/java/io/java-io-nio.html" target="_blank" rel="noopener noreferrer">Java NIO - 基础详解 | Java 全栈知识体系 (pdai.tech)<ExternalLinkIcon/></a></li>
<li><a href="https://blog.csdn.net/weixin_39327182/article/details/107252790" target="_blank" rel="noopener noreferrer">nio通信代码实例_一剑问九州的博客-CSDN博客_nio代码示例<ExternalLinkIcon/></a></li>
<li><a href="https://blog.csdn.net/l18637220680/article/details/79360451" target="_blank" rel="noopener noreferrer">NIO之Channel详解_我不是李大侠的博客-CSDN博客_nio的channel<ExternalLinkIcon/></a></li>
<li><a href="https://gitee.com/pic_bed_of_shiva/static-resources/raw/b6c87f51e22c327068bfbbb669bac48d7122a93f/showns/file/%E9%BB%91%E9%A9%ACio%E6%95%99%E6%9D%90.pdf" target="_blank" rel="noopener noreferrer">黑马Java-IO模式精讲(AIO&amp;BIO&amp;NIO) - PDF 教材<ExternalLinkIcon/></a></li>
</ul>
</div></template>
