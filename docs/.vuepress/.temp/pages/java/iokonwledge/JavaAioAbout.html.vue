<template><div><h2 id="开场废话" tabindex="-1"><a class="header-anchor" href="#开场废话" aria-hidden="true">#</a> <span id="开场废话">开场废话</span></h2>
<p><code v-pre>2022.04.15</code></p>
<p>Java 在 JDK 1.7 版本对 NIO 库做了升级，实现了完全异步通信的新 IO 套字节以及通道，这种新 IO 也被称为 AIO；</p>
<p>上面几篇也介绍过了 IO 的分类，以及IO都是需要操作系统支持的。</p>
<ul>
<li><strong>BIO 和 NIO 都是同步IO（服务端未完成操作时不返回），而 AIO 时操作系统层面实现的 异步IO</strong></li>
</ul>
<blockquote>
<p>异步IO则是采用 <strong>“订阅-通知”模式</strong> :  即应用程序向操作系统注册IO监听，然后继续做自己的事情。当操作系统发生IO事件，并且准备好数据后，在主动通知应用程序，触发相应的函数:</p>
</blockquote>
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/202204/java-io-aio-1.png" alt=""></p>
<blockquote>
<p><strong>Windows 内核中，实现了真正的异步IO支持，但是在 Linux 中没有很好实现异步IO，而是使用 Epoll 模拟（IO复用）</strong></p>
</blockquote>
<p>这里也可以插一句，为什么 Netty 用的是 NIO，而不是更高级的 AIO：</p>
<ul>
<li>AIO 在 UNIX 系统上不比 NIO 快（作者原话），因为 UNIX 没有很好实现嘛</li>
<li>Netty 整体架构是基本 reactor 模型，而 AIO 是 proactor 模型，混合在一起会比较混乱</li>
<li>AIO 接收数据需要预先分配缓冲区，NIO 需要接收时才需要分配缓存，所以对连接数量非常大但流量小的情况，内存浪费很多</li>
</ul>
<br/>
<h2 id="代码示例" tabindex="-1"><a class="header-anchor" href="#代码示例" aria-hidden="true">#</a> <span id="代码示例">代码示例</span></h2>
<p>服务端代码：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">AioServer</span> <span class="token punctuation">{</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">AioClient</span> <span class="token punctuation">{</span>
...
<ul>
<li><a href="https://www.pdai.tech/md/java/io/java-io-aio.html" target="_blank" rel="noopener noreferrer">Java AIO - 异步IO详解 | Java 全栈知识体系 (pdai.tech)<ExternalLinkIcon/></a></li>
<li><a href="https://blog.csdn.net/meiyongdesan/article/details/109034696" target="_blank" rel="noopener noreferrer">Netty 学习笔记（三）、AIO 简单介绍_吃饭睡觉胖胖胖的博客-CSDN博客_aio netty<ExternalLinkIcon/></a></li>
<li><a href="https://blog.csdn.net/madongyu1259892936/article/details/109668948" target="_blank" rel="noopener noreferrer">netty的基本概念-AIO详解_小马的学习笔记的博客-CSDN博客_aio netty<ExternalLinkIcon/></a></li>
<li><a href="https://www.cnblogs.com/mr-ao/p/14585898.html" target="_blank" rel="noopener noreferrer">Netty为什么放弃使用AIO - 米九虫 - 博客园 (cnblogs.com)<ExternalLinkIcon/></a></li>
</ul>
</div></template>
