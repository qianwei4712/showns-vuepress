...
<li><a href="#t4">Vector注意点</a>
<ul>
<li><a href="#t41">下标判断缺陷</a></li>
<li><a href="#t42">elements()</a></li>
<li><a href="#t43">线程安全初探</a></li>
</ul>
</li>
...
<p><em>Vector</em> 实现了 <em>List</em> 接口，是顺序容器，即元素存放的数据与放进去的顺序相同，允许放入 <code v-pre>null</code> 元素，底层通过<strong>Object 数组实现</strong>。</p>
<p><em>Vector</em> 与 <em>ArrayList</em> 基本相同，不过 <em>Vector</em> 一定程度上实现了同步。所以这篇源码介绍会比较简略，大致代码请转至 <strong>ArrayList 源码解析</strong> 。</p>
<p>学习方式为，将 <strong>Vector</strong> 源码以及相关类拷贝至自定义包内，进行注释添加，代码请移步：<br>
<a href="https://gitee.com/qianwei4712/JDK1.8.0.25-read/blob/master/src/main/java/java/util/Vector.java" target="_blank">https://gitee.com/qianwei4712/JDK1.8.0.25-read/blob/master/src/main/java/java/util/Vector.java</a></p>
<p>知识点总结如下：</p>
<p><img src="https://shiva.oss-cn-hangzhou.aliyuncs.com/picture-master/images/VectorSource.png" alt="VectorSource"></p>
<br>
...
<li>实现 <strong>Cloneable</strong> 接口，允许使用 <strong>clone()</strong> 方法克隆 --- 具体介绍请转 <strong>Java面向对象基础 - Object通用方法、枚举</strong></li>
...
<p>Vector 底层如下，经有3个字段，底层基于数组实现。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>和 ArrayList 不同，Vector 静态常量仅剩 MAX_ARRAY_SIZE ：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>    <span class="token comment">//数组最大容量，实际值为2^31-1-8，超出会爆OutOfMemoryError。</span>
...
<p><strong>Vector 构造时数组默认长度为 10；而 ArrayList 若不指定长度，则默认为0。</strong></p>
</blockquote>
<p>构造方法如下 ：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>    <span class="token comment">//无参构造器，重载 Vector(int initialCapacity)，默认长度10</span>
...
<h3 id="vector注意点" tabindex="-1"><a class="header-anchor" href="#vector注意点" aria-hidden="true">#</a> <span id="t4">Vector注意点</span></h3>
<p>这一部分就从简了，大部分都和 ArrayList 一样，有兴趣看下 ArrayList 篇吧。</p>
<p>以后会把有关集合的大部分比较重新开一篇汇总。</p>
<br>
<h4 id="vector下标判断的缺陷" tabindex="-1"><a class="header-anchor" href="#vector下标判断的缺陷" aria-hidden="true">#</a> <span id="t41">Vector下标判断的缺陷</span></h4>
<p>这里首先看下 ArrayList 的 下标位置判断方法：</p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>   <span class="token keyword">public</span> <span class="token keyword">synchronized</span> <span class="token keyword">void</span> <span class="token function">insertElementAt</span><span class="token punctuation">(</span><span class="token class-name">E</span> obj<span class="token punctuation">,</span> <span class="token keyword">int</span> index<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>再看下 Vector 的 addAll 方法，也是根据下标位置进行添加</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>    <span class="token comment">// 指定位置开始批量添加</span>
    ...
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>看看三个方法的索引判断</p>
<blockquote>
<p>同样是判断索引，为什么 ArrayList 判断了小于 0 ，而 Vector 没有？</p>
</blockquote>
<p>没有实践就没有发言权，一定是我错了，JDK是不可能错的！所以我做了个测试</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>妈耶，居然，报错了，而且报错行是在 <strong>System.arraycopy</strong> ，已经通过了索引判断，甚至扩容数组了。。。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>而且，这个只判断正整数不只出现在这一处，还出现在  <strong>remove、get</strong>  等，好几处主要方法。。。</p>
<p>然后，我百度了下，翻了3页才确定，貌似这里不判断负数，确实没有什么深意。。。</p>
<p>虽然这只是个牛角尖，但是，我得出一个重大的结论，JDK 也不是完美的。。。。</p>
<br>
<h4 id="elements" tabindex="-1"><a class="header-anchor" href="#elements" aria-hidden="true">#</a> <span id="t42">elements()</span></h4>
<p>这个方法代码也比较简单，只是因为 ArrayList 貌似没有。。。就着重看了下：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>    <span class="token comment">//枚举迭代器</span>
...
<h4 id="线程安全初探" tabindex="-1"><a class="header-anchor" href="#线程安全初探" aria-hidden="true">#</a> <span id="t43">线程安全初探</span></h4>
<p>虽然 Vector 在大多数的操作方法上加了锁，但是不是绝对的线程安全的。</p>
<p>在方法上加了 <strong>synchronized</strong> 效率极低，保证所有被修饰的方法对外只是保证单线程持有。</p>
<p><strong>以下部分节选自大佬的文章，这里就举下例子：<a href="https://blog.csdn.net/Fly_as_tadpole/article/details/86480371" target="_blank">https://blog.csdn.net/Fly_as_tadpole/article/details/86480371</a></strong></p>
<blockquote>
<p>虽然源代码注释里面说这个是线程安全的，因为确实很多方法都加上了同步关键字synchronized，但是对于复合操作而言，只是同步方法但并没有解决线程安全的问题。因为在两个原子操作之间存在间隙，在多线程环境中，完全有可能被其他线程获得 vector的 lock 并改变其状态。要真正达成线程安全，还需要以vector对象为锁，来进行操作。所以，如果是这样的话，那么用vector和ArrayList就没有区别了，所以，不推荐使用vector。</p>
</blockquote>
<blockquote>
...
<blockquote>
<p>单个的方法 synchronized 了并不代表组合（compound）的方法调用具有原子性，使 compound actions  成为线程安全的可能解决办法之一还是离不开intrinsic lock (这个锁应该是 vector 的，但由 client 维护)：</p>
</blockquote>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>   <span class="token comment">// Vector v = ...</span>
...
<ul>
<li><strong>在线程安全情况下要特别注意，Vector 虽然在方法上加锁，但是并非绝对线程安全。</strong></li>
<li><strong>很多博客最后的结论都是，不建议使用 Vector 。</strong></li>
</ul>
</div></template>
