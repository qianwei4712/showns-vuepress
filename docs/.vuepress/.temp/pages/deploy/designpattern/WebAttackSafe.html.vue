...
<p>在开始准备这一章的时候，对应用安全的理解仅限于依赖升级、SQL注入等。也是新项目要上线，才开始考虑这方面内容，基本上就是疯狂看大佬的总结（抄作业）。</p>
...
<br/>
<p>这里先汇总常见的攻击方式：</p>
<p><strong>OWASP（开放式web应用程序安全项目）</strong></p>
<ul>
...
<ul>
<li>XSS 是跨站脚本攻击(Cross Site Scripting)，恶意攻击者往Web页面里插入恶意Script代码，当用户浏览该页之时，嵌入其中Web里面的Script代码会被执行，从而达到恶意攻击用户的目的。</li>
</ul>
<br/>
...
<p>截至目前（2021.9.23），最新版的“10大漏洞”依然是2017版，应该快要出新版了。</p>
<ul>
...
<li>2017版下载备用地址：<a href="https://gitee.com/pic_bed_of_shiva/static-resources/blob/26d29676bf65626088949add19db8684fa083808/showns/file/OWASPTop102017v1.1.pdf" target="_blank" rel="noopener noreferrer">OWASPTop102017v1.1.pdf<ExternalLinkIcon/></a></li>
</ul>
...
<li>不安全的数据库配置，比如对查询集不合理处理，对sql查询语句错误时不当的处理，导致其错误信息暴露在前端</li>
<li>过于信任用户在前端所输入的数值，没有过滤用户输入的恶意数据，直接把用户输入的数据当做SQL语句执行</li>
</ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>若不对参数进行校验，就可能存在以下sql：</p>
...
<p><code v-pre>${}</code> 则将传入的数据直接显示生成在sql中，如：<code v-pre>where username=111</code> 。这种方式会存在 sql注入风险。</p>
...
<p>Java中 <code v-pre>System.Runtime.getRuntime().exec(cmd);</code> 可以在目标机器上执行命令，而构建参数的过程中可能会引发注入攻击。</p>
...
<ul>
<li>优点：使用方便，开发简单，一定程度上能预防CSRF攻击；</li>
<li>缺点：这种机制完全依托于浏览器，Referer字段容易被故意篡改，或者被禁用。</li>
</ul>
<ol start="2">
<li><strong>请求中添加token并验证</strong></li>
</ol>
<p>token 就是服务端颁发给客户端的一长串字符串。</p>
<blockquote>
<p>csrf 依赖于浏览器该问链接时自动对应网站的cookie带上，所以 token 不放 cookie 里，可以作为 query 参数每次携带，在处理请求前进行 token 验证即可。</p>
</blockquote>
<p>在基于没有其他漏洞会泄漏本次会话的token的设想下 ，黑客是无法获取用户的 token，<strong>所以一次会话可以使用同一个 token。</strong></p>
<ul>
<li>优点：安全程度比 Referer 的方式要高；</li>
<li>缺点：实现方式上稍微复杂（GET需要拼接链接，POST需要使用JS添加参数）；需要保证 token 存储的安全性。</li>
</ul>
<ol start="3">
...
<p>XSS 是跨站脚本攻击(Cross Site Scripting)，恶意攻击者往Web页面里插入恶意Script代码，当用户浏览该页之时，嵌入其中Web里面的Script代码会被执行，从而达到恶意攻击用户的目的。</p>
...
<blockquote>
<p>反射性xss 一般指攻击者通过特定的方式来诱惑受害者去访问一个包含恶意代码的URL。当受害者点击恶意链接url的时候，恶意代码会直接在受害者的主机上的浏览器执行。</p>
</blockquote>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>例如，我在一个博客中嵌入如下js，那么浏览博客的用户打开后，将会执行。</p>
<ol>
...
<h4 id="dom型-xss" tabindex="-1"><a class="header-anchor" href="#dom型-xss" aria-hidden="true">#</a> DOM型 XSS</h4>
<blockquote>
...
<ol>
<li>攻击者构造出特殊的URL、在其中可能包含恶意代码。例如：<code v-pre>http://xxx?name=&lt;script&gt;alert('aaa')&lt;/script&gt;</code></li>
...
<li>设置cookie httponly为true（具体请看下文的解释）</li>
</ul>
<p>具体方式如下：</p>
...
<li>后端， 推荐使用ApacheCommon包下 <code v-pre>StringEscapeUtils</code> – 用于正确处理转义字符，产生正确的Java、JavaScript、HTML、XML和SQL代码；</li>
</ul>
...
<p>这里举个例子，<strong>富文本的防御</strong>: 富文本的情况非常的复杂，js可以藏在标签里，超链接url里，何种属性里。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>所以我们不能过用上面的方法做简单的转义, 因为情况实在太多了。思路就是黑白名单校验，这里提供一个包，帮助我们去解析html树状结构，它使用起来和jquery非常的类似。</p>
...
console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span><span class="token function">xssFilter</span><span class="token punctuation">(</span><span class="token string">'&lt;div>&lt;font color="red">你好&lt;/font>&lt;a href="http://www.baidu.com">百度&lt;/a>&lt;script>alert("哈哈你被攻击了")&lt;/script>&lt;/div>'</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<p>CSP对你用于浏览页面的浏览器做出了限制，以确保它只能从可信赖来源下载的资源。资源可以是脚本，样式，图片，或者其他被页面引用的文件。这意味着即使攻击者成功的在你的网站中注入了恶意内容，CSP也能免于它被执行。</p>
<p>默认配置下不允许执行内联代码（``块内容，内联事件，内联样式），以及禁止执行eval() , newFunction() , setTimeout([string], ...) 和setInterval([string], ...) 。</p>
<ul>
<li>只允许本站资源</li>
</ul>
...
<h2 id="参考文章" tabindex="-1"><a class="header-anchor" href="#参考文章" aria-hidden="true">#</a> <span id="te">参考文章</h2>
...
<p><a href="https://blog.csdn.net/chest_/article/details/102537988" target="_blank" rel="noopener noreferrer">sql注入---入门到进阶_春日野穹-CSDN博客_sql注入条件<ExternalLinkIcon/></a></p>
<p><a href="https://www.cnblogs.com/jokmangood/p/11705850.html" target="_blank" rel="noopener noreferrer">mybatis是如何防止SQL注入的 - 王的微笑 - 博客园 (cnblogs.com)<ExternalLinkIcon/></a></p>
<p><a href="https://zhuanlan.zhihu.com/p/102151012" target="_blank" rel="noopener noreferrer">Mybatis是这样防止sql注入的 - 知乎 (zhihu.com)<ExternalLinkIcon/></a></p>
<p><a href="https://www.cnblogs.com/hyddd/archive/2009/04/09/1432744.html" target="_blank" rel="noopener noreferrer">浅谈CSRF攻击方式 - hyddd - 博客园 (cnblogs.com)<ExternalLinkIcon/></a></p>
<p><a href="https://www.jianshu.com/p/ffb99fc70646" target="_blank" rel="noopener noreferrer">web安全:CSRF攻击原理以及防御 - 简书 (jianshu.com)<ExternalLinkIcon/></a></p>
...
<p><a href="https://www.cnblogs.com/tugenhua0707/p/10909284.html" target="_blank" rel="noopener noreferrer">web安全之XSS攻击原理及防范 - 龙恩0707 - 博客园 (cnblogs.com)<ExternalLinkIcon/></a></p>
</div></template>
